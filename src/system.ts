...
export default class System {
    ...
    resetVelocities(temperature: number) {
        for (let i = 0; i < this.particles.count; i++) {
            this.velocities[3 * i + 0] = (Math.random() - 0.5) * temperature
            this.velocities[3 * i + 1] = (Math.random() - 0.5) * temperature
            this.velocities[3 * i + 2] = (Math.random() - 0.5) * temperature
        }
    }
}